import { TaskDecompositionEngine } from '../tools/task-decomposition-engine.js';
...
describe('TaskDecompositionEngine', () => {
    ...
    test('should analyze task complexity correctly', async () => {
        ...
        expect(result.complexity.overall).toBeGreaterThan(5); // Should be complex
    ...
    test('should not decompose simple tasks', async () => {
        const task: Task = {
            ...
            timeEstimate: 0.5 * 60 * 60 * 1000, // 30 minutes
            priority: 4
        };
