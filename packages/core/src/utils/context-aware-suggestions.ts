...
export function analyzeRequestContext(request: string): RequestContext {
  ...
  const workspaceMatch = request.match(
    /workspace[_\s]*id[:\s]*['"]*(\w+)['"]*|workspace[:\s]*['"]*(\w+)['"]/i
  );
  ...
  else if (lowerRequest.includes('workspace') || lowerRequest.includes('space'))
    target = 'workspace';
  ...
  if (
    ...
    lowerRequest.includes('now')
  ) {
    urgency = 'high';
  } else if (
    lowerRequest.includes('when you can') ||
    lowerRequest.includes('no rush') ||
    lowerRequest.includes('eventually')
  ) {
    urgency = 'low';
  }
  ...
  if (
    lowerRequest.includes('how do i') ||
    lowerRequest.includes('help me') ||
    lowerRequest.includes('guide') ||
    lowerRequest.includes('tutorial')
  ) {
    user_experience = 'beginner';
  } else if (
    lowerRequest.includes('efficiently') ||
    lowerRequest.includes('optimize') ||
    lowerRequest.includes('best way') ||
    lowerRequest.includes('fastest')
  ) {
...
2. \`create_chat_view_comment(view_id="found_id", comment_text="Your message")\``
      ...
      if (context.entities.view_id) {
        suggestion = {
          ...
          efficiency_notes: ['📖 Direct chat reading - most efficient', 'No navigation overhead'],
        ...
        suggestion = {
          ...
          efficiency_notes: ['🔍 Need to find chat first', 'Two-step process for reading'],
...
export function generateEfficiencyTips(context: RequestContext): string[] {
  ...
  if (context.target === 'chat') {
    tips.push(
      '💬 **Chat Efficiency**: Use find_chat_channels once, then save chat view IDs for future use'
    );
...
export function optimizeWorkflow(
  ...
  if (
    tools.includes('get_workspaces') &&
    tools.includes('get_spaces') &&
    tools.includes('get_views')
  ) {
    if (context.target === 'chat') {
