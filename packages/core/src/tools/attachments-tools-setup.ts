...
export function setupAttachmentsTools(server: McpServer): void {
  ...
  server.tool(
    ...
    {
      workspace_id: z.coerce.string().min(1).describe('The ID of the workspace (team)'),
      entity_type: AttachmentEntityTypeSchema.describe('The type of entity the attachments belong to (task or custom_field)'),
      entity_id: z.coerce.string().min(1).describe('The ID of the task or File custom field to list attachments for'),
