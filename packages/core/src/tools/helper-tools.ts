      ...
      context: z
        .object({
          known_ids: z
            .record(z.string())
            .optional()
            .describe('Any known IDs (workspace_id, space_id, etc.)'),
  ...
  server.tool(
    ...
    'Get a comprehensive overview of a workspace structure in a single efficient call. Better than multiple navigation calls.',
    {
      ...
      include_views: z
        .boolean()
        .default(false)
        .describe('Whether to include view details (slower but more complete)'),
