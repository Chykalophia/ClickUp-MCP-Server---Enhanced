...
export const AttachmentParentSchema = z.enum(['task', 'comment', 'doc', 'chat']);
...
export const GetAttachmentsFilterSchema = z.object({
  ...
  date_from: z
    .number()
    .optional()
    .describe('Filter attachments created after this date (Unix timestamp)'),
  date_to: z
    .number()
    .optional()
...
export const BulkAttachmentOperationSchema = z.object({
  operation: z
    .enum(['delete', 'move', 'copy', 'update_tags'])
...
export const generateAttachmentPreview = (
  attachment: AttachmentResponse
): {
  canPreview: boolean;
